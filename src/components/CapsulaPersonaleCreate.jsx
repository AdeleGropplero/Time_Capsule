...
const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < formData.files.length; i++) {
      data.append("files", formData.files[i]); // Aggiunge ogni file al FormData
    }

    try {
      const response = await fetch("http://localhost:8080/api/capsule", {
        method: "POST",
        body: data, // Qui invii i dati in formato multipart
      });

      if (response.ok) {
        alert("Capsula creata con successo!");
      } else {
        console.error("Errore nella creazione della capsula");
      }
    } catch (error) {
 ...
 ------------------------------------------------
 Il back end sarà più o meno così:
...
public class CapsuleController {
    ...
    public ResponseEntity<String> createCapsule(
        ...
        @RequestPart("openDate") String openDate,
        @RequestPart("email") String email,
        @RequestPart("files") List<MultipartFile> files) {

        // Qui puoi salvare i dati nel DB e gestire l'upload dei file
        capsuleService.saveCapsule(title, openDate, email, files);

        return ResponseEntity.ok("Capsula creata con successo!");
    }
}
